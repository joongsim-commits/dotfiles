...
description: >
  ...
  Enforces RED-GREEN-REFACTOR cycle. Triggers on "implement", "write code", "add feature",
  "fix bug", "build", "create function", "add endpoint", "write test first", "TDD".
---
...
```typescript
// ✅ Just enough to pass
async function retryOperation(fn: () => Promise<T>): Promise<T> {
  for (let i = 0; i < 3; i++) {
    try { return await fn(); }
